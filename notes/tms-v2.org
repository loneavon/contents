...
下表中的 *分时图* 使用了 [[http://omnipotent.net/jquery.sparkline/][jQuery Sparklines]] ，这是一个绘制行内图表的JS库。这
种从分时图可以看出每天耗时的具体分布，红色是工作，蓝色是学习（出CP），绿色
是生活，黑色是其他。这几种颜色的对比还是相当鲜明的，这就使得很多信息能够一
眼看出来。
